...
(function () {
    ...
    p._generatedId = typeof(localStorage) ? ((+localStorage.getItem('_generatedId') || -1) - 1) : -1;
...
(function () {
            ...
            if (_isLocalStorageEnabled) {
                ...
                if (draftData) {
                    ...
                    if (confirm('检测到您有【未保存】的草稿，时间在' + draftDataDate.toString() + ', 是否恢复?')) {
                        ...
                        localStorage.setItem('_generatedId', -1);
                        rap.project._generatedId = -1;
                    }
                }
            }
            ...
            window.onbeforeunload = function () {
                ...
                if (_isEditMode) {
                    if (_isLocalStorageEnabled) {
                        ...
                        localStorage.setItem('_generatedId', p.curId);
     ...
     * edit
     */
    ws.edit = function (id, key, editMode) {
        var el,
            str = "",
            oldValue,
    ...
    ws.dataTypeSelectChanged = function (parameterId, value) {
        console.log('dataTypeSelectChanged',parameterId,value);
        p.setParameter(parameterId, value, "dataType");
        this.checkIsCustomJson(value,parameterId);
        this.switchA(_curActionId);
    };

    ws.checkIsCustomJson = function (value,parameterId) {


        var isCustomJson = false;
        var cusData;
        // if(_customJsonData&&_customJsonData.length>0){
        //     for(var ci = 0;ci<_customJsonData.length;ci++){
        //         var cData = _customJsonData[ci];
        //         if(value==cData.name){
        //             isCustomJson =true;
        //             this._doesImportToRequest = true;
        //
        //             var data = eval("(" + cData.data + ")");
        //             if (data instanceof Array) {
        //                 data = data[0];
        //             }
        //
        //             processJSONImportMethod(data,value,parameterId);
        //             this.switchA(_curActionId);
        //         }
        //     }
        // }
        ...
        if(isCustomJson){
            window.parameterId=parameterId;
            param.parameterList = cusData;
            // doImportCustomJson(value, cusData);
        } else {
            param.parameterList = [];
        }
        return isCustomJson;
    ...
    function doImportCustomJson(value, data) {
        this._doesImportToRequest = true;
        data = data || eval("(" + window.customJson + ")");
        if (data instanceof Array) {
            data = data[0];
        }
        processJSONImportMethod(data, value || window.customJsonName, window.parameterId);
        ws.switchA(_curActionId);
    }
    ...
    function processJSONImportMethod(f, k, pId, notFirst,arrContext, notExsit) {
        var id, param;
        var doesImportToRequest = ws._doesImportToRequest;//判断是在请求参数中加入json，还是返回参数中加入
        if (notFirst) {
            if (!pId) {//是否有插入的节点
                if (!notExsit) {
                    if (doesImportToRequest === true) {
                        id = p.addRequestParameter(_curActionId);//在请求参数中加入json,并获取actionId
                    } else {
                        id = p.addResponseParameter(_curActionId);//在返回参数中加入json,并获取actionId
                    }
                    param = p.getParameter(id);//获取当前插入点的参数
                    param.identifier = k;//
                } else {
                    param = newParameter();
                    id = param.id;
        ...
        if (f instanceof Array && f.length) {
            if (notFirst) {
                ...
                if (typeof f2 === 'string') {
                    ...
                    for (key in f2) {
                        processJSONImportMethod(f2[key], key, notExsit ? param : (notFirst ? id : undefined), true, notExsit ? undefined : (f.length > 1 ? f : undefined), notExsit);
                    }
                }
            ...
            Object.keys(f).forEach(function (key) {
                ...
                processJSONImportMethod(f[key], key, notExsit ? param : (!notFirst?pId:id), true, undefined, notExsit);
            });
        }
    ...
    ws.processJSONImportMethod = processJSONImportMethod; // change json data
     ...
     * 显示自定义json modal
     */
    ws.importCustomJSON = function () {
        $('#select-json-modal').show();
        $('#select-json-modal').addClass('in');

    }

    ws.hideCustomJsonModal = function () {
        $('#select-json-modal').removeClass('in');
        $('#select-json-modal').addClass('out');
        $('#select-json-modal').hide();
    }

    ws.addCustomJsonModal = function () {
        ws.hideCustomJsonModal();
        console.log("select json obj:",window.customJson);
        console.log("curActionId",_curActionId,_curModuleId);
     ...
     */
    ws.showOpColumn = function (event, pId, editMode) {
        var isEditMode = _isEditMode;
        if (typeof editMode === 'boolean') {
            isEditMode = editMode;
        }
     ...
     */
    ws.hideOpColumn = function (event, pId, editMode) {
        var isEditMode = _isEditMode;
        if (typeof editMode === 'boolean') {
            isEditMode = editMode;
        }
    ...
    // todo processJSONImport

    /**
     * process JSON import
     *
     * @param {object}  f value to be processed  要加入的数据，是一个对象（从json演变过来）
     * @param {string}  k key　
     * @param {number}  pid parameter id　要插入的节点位置
     * @param {boolean} notFirst　是否是第一次
     */
     ...
     */
    function getPTRHtml(param, level, editMode, readonly) {
        ...
        str += getPTDHtml(param.id, util.escaper.escapeInH(param.identifier), "identifier", level, isEditMode, readonly);
        str += getPTDHtml(param.id, util.escaper.escapeInH(param.name), "name", undefined, isEditMode, readonly);
        str += getDataTypeEditSelectHtml(param.id, param.dataType, isEditMode, readonly);
        // for remarkFilter, escape after filter processed...
        str += getPTDHtml(param.id, param.remark, "remark", undefined, isEditMode, readonly);
        str += "</tr>";

        for (var i = 0; i < parameterListNum; i++) {
            str += getPTRHtml(parameterList[i], level + 1, isEditMode, subReadonly);
        }

        return str;
    }

    ws.getPTRHtml = getPTRHtml; // to generate the table
     ...
     * for remarkFilter, escape after filter processed...
     */
    function getPTDHtml(id, value, type, level, editMode, readonly) {
        var isEditMode = _isEditMode;
        if (typeof editMode === 'boolean') {
            isEditMode = editMode;
        }
        ...
        return "<td id='td-param-" + type + "-" + id + "' class='td-param " + type +
            ...
            "\","+isEditMode+");' >" + (level ? new Array(level + 1).join('&nbsp;&nbsp;&nbsp;&nbsp;') : '') + value + "</td>";
    }

    ws.getPTDHtml = getPTDHtml;
     ...
     */
    function getDataTypeEditSelectHtml(id, type, editMode, readonly) {
        var str = "";
        var isEditMode = _isEditMode;
        if (typeof editMode === 'boolean') {
            isEditMode = editMode;
    ...
    ws.getDataTypeEditSelectHtml = getDataTypeEditSelectHtml;
